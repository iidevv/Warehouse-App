...
export const getInventoryProducts = async (
  ...
  if ((vendor_id, name)) {
    const product = await hhInventoryModel.findOne({
      vendor_id,
      product_name: name,
    });
    return product;
    ...
    const total = await hhInventoryModel.countDocuments(query);
    const totalPages = Math.ceil(total / pageSize);
    const skip = (page - 1) * pageSize;

    const sort = isUserRequest ? { last_updated: -1 } : { inserted_at: 1 };

    const Inventory = await hhInventoryModel
      .find(query)
      .sort(sort)
      .skip(skip)
      .limit(pageSize);
...
export const addInventoryProduct = async (productData) => {
  ...
  const Inventory = await hhInventoryModel.findOne({
    product_name,
  });

  if (Inventory) throw new Error("Product already exists!");

  const newProduct = new hhInventoryModel({
    vendor,
    vendor_id,
    bigcommerce_id,
    price,
...
router.delete("/products", async (req, res) => {
  ...
  const Inventory = await hhInventoryModel.findOneAndDelete({ bigcommerce_id });
  if (!Inventory) return res.json({ message: "Product doesn't exists!" });

  res.json({ message: "Deleted Successfully" });
});

export { router as hhInventoryRouter };
